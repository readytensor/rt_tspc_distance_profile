...
The window length is calculated as a function of the minimum series length in the training data. The window length is calculated as follows:

```
window_length = int(log2(min_series_length) * window_length_factor)
```

where `window_length_factor` is a tunable parameter. Recommended range is between 3 and 8. Default value is 5.

#### Number of Nearest Neighbors

The number of nearest neighbors (windows in training data that are most similar to any given window in test data) is a tunable parameter. Recommended range is between 1 and 5. Default value is 3. The predicted label for each time-step is averaged across the labels of the corresponding time-steps within the nearest neighbors windows.

Recommended range is between 3 and 10. Default value is 8.
